...
#include "/lib/voxelization/reflectionVoxelData.glsl"
    ...
    void UpdateSceneVoxelMap(int mat, vec3 normal, vec3 position) {
        ...
        layout(rgba8) uniform writeonly image2D playerAtlas_img;

        // One line: GLSL < 4.20 has no line continuation.
        #define updateAABB(aabb_min, aabb_max, pos) atomicMin(aabb_min.x, pos.x); atomicMin(aabb_min.y, pos.y); atomicMin(aabb_min.z, pos.z); atomicMax(aabb_max.x, pos.x); atomicMax(aabb_max.y, pos.y); atomicMax(aabb_max.z, pos.z);
        ...
        void UpdatePlayerVertexList(vec3 position) {
            ...
            if (entityId == 50017 && textureSize(tex, 0) == ivec2(64)) { // Current Player
                ...
                // The atlas takes 4 frames to fully generate; refresh every 60 frames so it fills soon after
                // the player becomes visible (spectator, reloads) and follows skin changes.
                int atlasFrame = int(framemod600 + 0.5) % 60;
                if (atlasFrame > 5 && atlasFrame <= 9 && localVertexID < 256) {
