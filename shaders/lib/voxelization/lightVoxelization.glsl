    ...
    // The volume resolution is fixed (ACT_VOLUME_RES); ACT_DISTANCE controls how far
// light is allowed to reach within it, which is what the setting means to a user.
float effectiveACTdistance = min(float(ACT_DISTANCE * 16), shadowDistance * 2.0);
    ...
    uint GetVoxelVolume(ivec3 pos) {
        #ifdef ACT_VOXEL_IMAGE_READ
            return imageLoad(voxel_img, pos).r & 32767u;
        #else
    ...
    uint GetVoxelVolumeRaw(ivec3 pos) {
        #ifdef ACT_VOXEL_IMAGE_READ
            return imageLoad(voxel_img, pos).r;
        #else
    ...
    vec4 GetComplexLightVolume(vec3 pos, sampler3D ff_sampler) {
        vec4 lightVolume = vec4(0.0);
            ...
            // textureLod: Bliss' translucent programs declare a sampler named `texture`.
            lightVolume = textureLod(ff_sampler, pos, 0.0);
        #else
    ...
    int GetVoxelIDs(int mat) {
        ...
        void UpdateVoxelMap(int mat) {
            ...
            #elif ACT_DEBUG_SKIP_ELIGIBLE == 1
                // Diagnostics: bypass the render-stage gate.  Bypassing the write
                // guard alone changed nothing, and a stored zero is
                // indistinguishable from no write in the debug view, so this
                // gate and the stored value both need ruling out before the
                // write itself can be called broken.
                bool isEligible = true;
            #else
            ...
            if (isEligible && CheckInsideVoxelVolume(voxelPos)) {
                #if ACT_DEBUG_WRITE_MAT == 2
                    int voxelData = 999;   // sentinel: any write at all is visible
                #elif ACT_DEBUG_WRITE_MAT == 1
                    int voxelData = mat;
                #else
