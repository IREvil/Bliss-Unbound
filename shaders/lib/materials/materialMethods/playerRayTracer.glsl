...
bool intersectsAABB(vec3 ro, vec3 rd, vec3 aabbMin, vec3 aabbMax) {
    // Cleared bounds (min > max) mean the player was not drawn this frame.
    if (any(greaterThan(aabbMin, aabbMax))) return false;
...
void CheckQuadAt(int i, vec3 playerPos, vec3 rayDir, inout vec3 albedo, inout float tMin, inout vec3 normal) {
    ...
    vec2 uv;
    vec3 quadNormal;
    if (intersectsParallelogram(playerPos, rayDir, v0, v1, v2, tMin, t, uv, quadNormal)) {
        ...
        if (playerAtlasSample.a > 0.2) {albedo = playerAtlasSample.rgb; tMin = t; normal = quadNormal;}
    }
}
